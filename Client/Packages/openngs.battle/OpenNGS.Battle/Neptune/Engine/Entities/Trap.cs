...
namespace Neptune
{
    ...
    public class Trap : Effect
    {
        ...
        private Skill startSkill;
        private List<Actor> inRangeRoleList;
        private Skill enterSkill;
        private Skill leaveSkill;
        private Skill triggerSkill;
        private Skill effectSkill;
        private List<Actor> leaveAffectRoleList;
        ...
        private List<Effect> casts = null;
        ...
        public void Create(int trapId, Skill skill, Vector2 position, Actor target = null)
        {
            base.Create();

            this.Type = EffectType.Trap;
            this.Skill = skill;
            this.Side = skill.Caster.Side;
            this.trapId = trapId;
            this.Position = position;
            targetRole = target;
            this.Target = target;
            this.TrapData = NeptuneBattle.Instance.DataProvider.GetTrapData(trapId);//.Clone();
            this.Skill.Caster.RoleSkin.RoleSkinTrapReplace(this.TrapData);
            ...
            this.shapeChangeRatio = UFloat.Round(TrapData.ShapeChangeRatio * NeptuneConst.Hundredth);
            this.ignoreShapeChangeRatio = TrapData.IgnoreShapeChangeRatio;
            this.shapeChangeTime = UFloat.Round(TrapData.ShapeChangeTime * NeptuneConst.Milli);
            this.shapeChangeTimeDelta = this.shapeChangeTime;
            this.ignoreShapeChangeTime = UFloat.Round(TrapData.IgnoreShapeChangeTime * NeptuneConst.Milli);
            ...
            if (dir != NeptuneConst.Vector2Zero)
                this.Orientation = dir;
            else
                this.Orientation = this.Skill.Caster.Orientation;
            ...
            if(TrapData.RemoveAbilitys != null)
            {
                this.RemoveAbilitys = TrapData.RemoveAbilitys.ToList();
            }
            ...
            this.effectInterval = UFloat.Round(TrapData.EffectInterval * NeptuneConst.Milli);
            this.positionOffset = this.TrapData.PosOffset != null && this.TrapData.PosOffset.Length > 1 ? new Vector2(this.TrapData.PosOffset[0], this.TrapData.PosOffset[1]) : NeptuneConst.Vector2Zero;
            this.Height = this.TrapData.PosOffset != null && this.TrapData.PosOffset.Length > 2 ? this.TrapData.PosOffset[2] : 0;
            this.effectSkill = null;
            triggerTimer = 0;
            inRangeRoleList = new List<Actor>();
            started = false;
            if (this.effectAction != null)
                this.casts = new List<Effect>();
            else
                this.casts = null;
            this.AddToCaster();
            this.Source = skill.Caster;

#if BATTLE_LOG
            if (EngineGlobal.BattleLog)
                NeptuneBattle.log("{0}: {1} --> {2} crete Trap {3}  startAction:{4}", this.Skill.Caster.FullName, this.ToString(), this.Target != null ? this.Target.FullName : "", this.trapId, this.startAction);
#endif
        }

        public override void Delete()
        {
            ObjectPool<Trap>.Delete(this);
        }
        ...
        private void OnStop(bool endAction = true)
        {
            ...
            {
                IAgent effectJoint = NeptuneBattle.PlayEffect(this.endEffect, this.Position, this.Orientation, this.Height, this.ZOrder, null, null, this.Skill.Caster);
        ...
        private void CreateTrapEffect()
        {
            ...
            //this.joint = NeptuneBattle.PlayEffect(this.effectName, this.Position, this.Orientation, 0, this.ZOrder, this);
        ...
        private void OnControllerCreate(IEffectController go)
        {
            ...
            if (TrapData.ShowIndicatorSide != RelativeSide.Both && NeptuneBattle.Instance.RuleManager != null 
                && TrapData.ShowIndicatorSide != this.Skill.Caster.GetRelation(NeptuneBattle.Instance.RuleManager.GetCurrentPlayer().CurrentRole))
                return;
            int type = 0;
            float[] indicatorParams = new float[2];
            indicatorParams[0] = shapeParam1;
            indicatorParams[1] = shapeParam2;
            ...
            go.AddFloorEffect(Skill.Caster, type, indicatorParams);
        }
        ...
        private void RecordPositionOffset()
        {
            switch (this.followType)
            {
                ...
                case 2:
                    // 跟随释放者
                    this.posOffset = this.Position - this.Skill.Caster.Position;
                    break;
                default:
                    break;
            }
        ...
        private void UpdatePosition(float dt)
        {
            switch (this.followType)
            {
                ...
                case 2:
                    // 跟随释放者
                    this.Position = this.Skill.Caster.Position + posOffset;
                    break;
            }
        ...
        private void PerformStartAction()
        {
            ...
            startSkill = this.Skill.Caster.GetSkillById(startAction);
            if (startSkill == null)
            {
        ...
        private void DetectTargetsInRange()
        {
            ...
            Skill enterSkill = null;
            List<Actor> newList = new List<Actor>();
            //        foreach (Actor role in NeptuneBattle.Instance.GetSurvivors(this, this.affectSide, shapeParam1+ shapeParam2 + 500))
            foreach (Actor role in NeptuneBattle.Instance.AOIManager.GetSurvivors(this, this.affectSide, shapeParam1 + shapeParam2 + 500))
            {
                if (this.TrapData.SelfTrigger && role != this.Skill.Caster)
                    continue;
            ...
            foreach (Actor role in inRangeRoleList)
            {
        ...
        private void PerformTriggerAction(float dt)
        {
            ...
            if (triggerTimer >= triggerInterval)
            {
                ...
                {
                    triggerSkill = this.Skill.Caster.GetSkillById(triggerAction);
                    if (triggerSkill == null)
                    {
        ...
        private void PerformEndAction()
        {
            ...
            Skill endSkill = this.Skill.Caster.GetSkillById(endAction);
            if (endSkill == null)
            {
        ...
        private Skill OnEnterRange(Actor role)
        {
            if (enterAction == 0)
            {
                return null;
            ...
            Skill enterSkill = Skill.Caster.GetSkillById(enterAction);
            if (enterSkill == null)
            {
            ...
            ResultType result = enterSkill.CanUse(role);
            if (result == ResultType.Success || result == ResultType.TooFar || result == ResultType.TooNear || result == ResultType.OutOfSightRange)
            {
                if (this.TrapData.EffectPointType == EffectPointType.TrapPoint)
                    enterSkill.AttackPointElement = this;
                if (enterSkill.Data.TargetType == TargetType.Position)
                {
                    enterSkill.Start(this.Position);
                }
                else
                    enterSkill.Start(role);
            }
            return enterSkill;
        }

        private bool CheckTriggerType(Actor role)
        {
            if (role == null)
                return false;
            ...
            if ((this.triggerType & TriggerType.HPUnder) > 0 && role.HP <= UFloat.Round(role.MaxHP * (this.triggerParam * NeptuneConst.Hundredth)))
            {
                return true;
            }
            if ((this.triggerType & TriggerType.MpUnder) > 0 && role.MP <= UFloat.Round(role.MaxMP * (this.triggerParam1 * NeptuneConst.Hundredth)) && role.MaxMP > 0)
            {
                return true;
            }
            return false;
        ...
        private void OnLeaveRange(Actor role)
        {
            if (this.RemoveAbilitys != null)
            {
                ...
                for (int i = 0; i < this.RemoveAbilitys.Count; i++)
                {
                    ...
                    if (actionID == startAction && startSkill != null)
                    {
                        role.RemoveSkillAbility(startSkill);
                    }
                    if (actionID == enterAction && enterSkill != null)
                    {
                        role.RemoveSkillAbility(enterSkill);
                    }
                    if (actionID == triggerAction && triggerSkill != null)
                    {
                        role.RemoveSkillAbility(triggerSkill);
                    }
                }
            }
            if (leaveAction == 0)
            ...
            Skill leaveSkill = Skill.Caster.GetSkillById(leaveAction);
            if (leaveSkill == null)
            {
                Logger.LogError("No Enter Skill Found From Caster Skill list, ID: " + enterAction);
                return;
            }
            ResultType result = leaveSkill.CanUse(role);
            if (result == ResultType.Success || result == ResultType.TooFar || result == ResultType.TooNear || result == ResultType.OutOfSightRange)
            {
                if (this.TrapData.EffectPointType == EffectPointType.TrapPoint)
                    leaveSkill.AttackPointElement = this;
                if (leaveSkill.Data.TargetType == TargetType.Position)
                {
                    leaveSkill.Start(this.Position);
                }
                else
                    leaveSkill.Start(role);
            }
        }

        // 清除角色身上开始技能施加的Buff
        private void ClearStartAction(Actor role)
        {
            if (startSkill == null)
            {
                return;
        ...
        // 清除角色身上进入技能施加的Buff
        private void ClearEnterAction(Actor role)
        {
            if (enterSkill == null)
            {
                return;
        ...
        // 清除角色身上周期技能施加的Buff
        private void ClearTriggerAction(Actor role)
        {
            if (triggerSkill == null)
            {
                return;
        ...
        private void CheckLifeTime(float dt)
        {
            ...
            if ((this.followType == 1 && (this.Target == null || this.Target.IsDead)) || (this.followType == 2 && (this.Skill.Caster == null || this.Skill.Caster.IsDead)))
            {
                //当跟踪目标死亡时 结束陷阱
                this.lifeTime = 0;
            }
            if (this.lifeTime <= 0.0 && this.Skill.Data.ContinuousType != ContinuousType.Activate)
            {
                this.OnStop();
            }
        }
        ...
        // 查看目标角色是否在该陷阱的影响范围内
        private bool WithinArea(Actor role)
        {
            bool result = false;
        ...
        {
            get { return string.Format("[{0}{1}]{2}>Trap", this.SideSign, !this.Running ? "D" : "", this.Skill.FullName()); }
        }
        ...
        public override void Stop()
        {
#if BATTLE_LOG
            if (EngineGlobal.BattleLog)
                NeptuneBattle.log("{0}: {1} --> {2} stop Trap {3}  startAction:{4}", this.Skill.Caster.FullName, this.ToString(), this.Target != null ? this.Target.FullName : "", this.trapId, this.startAction);
#endif
            ...
            {
                foreach (Effect effect in this.casts)
                {
        ...
        private void CastTrapSkill(Skill skill)
        {
            Actor target = null;
            ...
            {
                target = this.Skill.Caster.Target;
            }
            else if (this.targetType == TargetType.Self)
            {
                target = this.Skill.Caster;
            }
            skill.CastPosition = this.Position;
            ResultType result = skill.CanUse(target, false, true);
            ...
            {
                skill.Orientation = this.Orientation;
                if (this.TrapData.EffectPointType == EffectPointType.TrapPoint)
                    skill.AttackPointElement = this;
                if (skill.Data.TargetType != TargetType.Position && skill.Data.TargetType != TargetType.Direction)
                {
                    skill.Start(target, this.Position);
                }
                else
                {
                    skill.Start(this.Position);
                }
        ...
        public void CastManualAction()
        {
            if (manualAction > 0)
            {
                Skill manualSkill = Skill.Caster.GetSkillById(manualAction);
                if (manualSkill == null)
                {
                    Logger.LogError("No manualSkill Found From Caster Skill list, ID: " + manualAction);
                    return;
                }
                if (this.TrapData.UseTrapRange)
                    manualSkill.MaxRange = this.shapeParam1;
                CastTrapSkill(manualSkill);
        ...
        {//TODO：  迁移到  OrcaSimulator中
            UVector2 hit = NeptuneConst.Vector2Zero;
        ...
        private void createRoundEffect()
        {
            ...
            effectSkill = this.Skill.Caster.GetSkillById(effectAction);
            if (effectSkill == null)
            {
            ...
            if (effectSkill.Data.FlyCastDirections != null && effectSkill.Data.FlyCastDirections.Length > 0)
            {
                for (int i = 0; i < effectSkill.Data.FlyCastDirections.Length; i++)
                {
                    int angle = effectSkill.Data.FlyCastDirections[i];
                    Effect cast = effectSkill.CreateCast();
                    cast.SetFollowElement(this);
                    cast.effectAngle = angle;
                    cast.Position = nextPositon(0, ref cast.effectAngle, cast.CasterDistance, effectSkill.Data.FlyRoundSpeed) + this.Position;
                    this.casts.Add(cast);
                    NeptuneBattle.Instance.AddEffect(cast);
                    effectSkill.AddMarkCount(cast.EffectIndex);
                }
        ...
        private void updateEffect(float dt)
        {
            ...
            for (int i = 0; i < this.casts.Count; i++)
            {
                ...
                if (cast.IsFollow)
                {
                    UVector2 nextPos = nextPositon(dt, ref cast.effectAngle, cast.CasterDistance, effectSkill.Data.FlyRoundSpeed);
                    cast.Position = nextPos + this.Position;
                    if (this.intervalTime <= 0)
                    {
                        Actor role = effectSkill.FindTarget(null);
                        if (role != null && !role.IsDead)
                        {
                            cast.SetFollowElement(null);
                            cast.Target = role;
                            cast.InitSpeed();
                            if (effectSkill.Data.FlyBounce > 0)
                            {
                                cast.SetBounce(effectSkill.Data.FlyBounce);
                            }
                            if (effectSkill.Data.FlyTrackingTarget)
                            {
                                cast.TrackTarget(role);
                            }

                            if (!string.IsNullOrEmpty(effectSkill.Data.DirectEffect))
                            {
                                NeptuneBattle.PlayEffect(effectSkill.Data.DirectEffect, cast.Position, cast.Speed.normalized, cast.Height, effectSkill.Data.DirectOrder);
                            }
