        ...
        private object RootDictionary(object parse, Type type)
        {
            ...
            if (parse is Dictionary<string, object>)
            {
                ...
                foreach (var kv in (Dictionary<string, object>)parse)
                {
					...
					if (kv.Value is Dictionary<string, object> || kv.Value is IList)
					{
						v = RootDictionary(kv.Value,gtypes[1]);
						if(v==null)
                            ...
                            default:
                                {
                                    ...
                                    {
                                        throw new Exception(pi.Name + "[" + pi.pt.ToString() + "] Value Type Error: " +  v.ToString(), ex);
                                    }
#endif
                                    break;
                                }
        ...
        private object CreateGenericList(List<object> data, Type pt, Type bt, Dictionary<string, object> globalTypes)
        {
            ...
            foreach (object ob in data)
            {
                ...
                else
				{
						...
						else if(bt == typeof(int))
						{
							value = Convert.ToInt32(value);
						}
                        else if(bt.IsEnum)
                        {
                                value = CreateEnum(bt, value);
                        }
