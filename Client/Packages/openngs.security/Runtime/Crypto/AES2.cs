...
public class Aes2
{
    ...
    public Aes2(KeySize keySize, byte[] keyBytes)
    {
        SetNbNkNr(keySize);
    ...
    private void ShiftRows()
    {
        ...
        for (int r = 0; r < 4; ++r)  // copy State into oldShield[]
        {
            for (int c = 0; c < 4; ++c)
            {
        ...
        for (int r = 1; r < 4; ++r)  // shift oldShield into State
        {
            for (int c = 0; c < 4; ++c)
            {
    ...
    /// <returns></returns>
    public static byte[] AESEncrypt(byte[] scrData, string keyStr, string IVstr, bool enCrypt)
    {
        byte[] outbuffer;
        ...
        keyArr = Encoding.ASCII.GetBytes(keyStr); ;
        byte[] IV1 = new byte[16];
        Encoding.Unicode.GetBytes(IVstr).CopyTo(IV1, 0);
        Aes2.KeySize keysize = Aes2.KeySize.Bits128;
        Aes2 aes1 = new Aes2(keysize, keyArr);
        byte[] iv = new byte[16];
        IV1.CopyTo(iv, 0);
        ...
        if (enCrypt)
        {
            outbuffer = new byte[(scrData.Length + 15) / 16 * 16];
        }
        else
        {
            outbuffer = new byte[scrData.Length];
        }


        for (int i = 0; i < scrData.Length;)
        {
            for (int j = 0; j < 16; ++j, ++i)
            {
                if (i < scrData.Length)
                    cipherText[j] = scrData[i];
                else
                    cipherText[j] = 0;
            }
            if (enCrypt)
            {//加密
                XorText(cipherText, iv);
                aes1.Cipher(cipherText, decipheredText);
                decipheredText.CopyTo(iv, 0);
                decipheredText.CopyTo(outbuffer, i - 16);
            }
            else
            {//解密
                aes1.InvCipher(cipherText, decipheredText);
                XorText(decipheredText, iv);
                cipherText.CopyTo(iv, 0);
                decipheredText.CopyTo(outbuffer, i - 16);
        ...
        if (!enCrypt)
        {
            int length = BitConverter.ToInt32(outbuffer, 0);
            byte[] result = new byte[length];
            Array.Copy(outbuffer, 4, result, 0, length);
            return result;
        }
