...
namespace OpenNGS.UI
{
    ...
    public class HorizontalRecyclingSystem : RecyclingSystem
    {
        ...
        private Dictionary<int, int> m_CellIndexDic = new();
        public Dictionary<int, int> CellIndexDic => m_CellIndexDic;
        #region INIT
        ...
        private void CreateCellPool()
        {
            ...
            while ((poolSize < minPoolSize || currentPoolCoverage < requriedCoverage) && poolSize < DataSource.GetItemCount())
            {
                ...
                OnInstantiateCell?.Invoke(item.GetComponent<ICell>());
                OnCreateCell?.Invoke(item);
                item.name = "Cell";
                ...
                OnScrollCell?.Invoke(poolSize);
                m_CellIndexDic[_cachedCells.Count - 1] = poolSize;
        ...
        public override Vector2 OnValueChangedListener(Vector2 direction)
        {
            ...
            {
                ret = RecycleLeftToRight();
            }
            else if (direction.x > 0 && _cellPool[leftMostCellIndex].MaxX() > _recyclableViewBounds.min.x)
            {
                ret =  RecycleRightToleft();
            }
            
            CalculateMostLeftCellInView(direction.x);
            
            return ret;
        }

        private int m_MostLeftCellIndexInView = 0;
        public int MostLeftCellIndexInView => m_MostLeftCellIndexInView;

        private void CalculateMostLeftCellInView(float direction)
        {
            // todo:
            // support grid mode
            ...
            for (var i = 0; i < _cellPool.Count; ++i)
            {
                var index = (leftMostCellIndex + i) % _cellPool.Count;
                var cell = _cellPool[index];
                var pos = Viewport.InverseTransformPoint(cell.position);

                if ((direction <= 0f && Mathf.Abs(pos.x) <= cell.sizeDelta.x) || (direction > 0f && pos.x >= 0f))
                {
                    if (m_MostLeftCellIndexInView != m_CellIndexDic[index])
                    {
                        m_MostLeftCellIndexInView = m_CellIndexDic[index];
        ...
        private Vector2 RecycleLeftToRight()
        {
            ...
            while (_cellPool[leftMostCellIndex].MaxX() < _recyclableViewBounds.min.x && currentItemCount < DataSource.GetItemCount())
            {
                ...
                OnScrollCell?.Invoke(currentItemCount);
                m_CellIndexDic[leftMostCellIndex] = currentItemCount;
        ...
        private Vector2 RecycleRightToleft()
        {
            ...
            while (_cellPool[rightMostCellIndex].MinX() > _recyclableViewBounds.max.x && currentItemCount > _cellPool.Count)
            {
                ...
                OnScrollCell?.Invoke(currentItemCount - _cellPool.Count);
                m_CellIndexDic[rightMostCellIndex] = currentItemCount - _cellPool.Count;
