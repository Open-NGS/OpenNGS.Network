...
public class OpenNGSTabView : VisualElement
{
    public new class UxmlFactory : UxmlFactory<OpenNGSTabView, UxmlTraits> { }

    private const string k_styleName = "NGSTabView";
    ...
    private readonly List<OpenNGSTabButton> m_Tabs = new List<OpenNGSTabButton>();
    private OpenNGSTabButton m_ActiveTab;

    public override VisualElement contentContainer => m_Content;

    public event Action<VisualElement> OnSelect;

    public OpenNGSTabView()
    {
        AddToClassList(s_UssClassName);

        styleSheets.Add(Resources.Load<StyleSheet>($"{k_styleName}"));
    ...
    public void AddTab(OpenNGSTabButton tabButton, bool activate)
    {
    ...
    public void RemoveTab(OpenNGSTabButton tabButton)
    {
        ...
        if((m_ActiveTab == null) && m_Tabs.Any())
        {
            ...
            OpenNGSTabButton tabToActivate = m_Tabs[clampedIndex];
    ...
    private void ProcessEvent(AttachToPanelEvent e)
    {
        ...
        for (int i = 0; i < m_Content.childCount; ++i)
        {
            ...
            if (element is OpenNGSTabButton button)
            {
        ...
        {
            m_ActiveTab = (OpenNGSTabButton)m_TabContent[0];

            SelectTab(m_ActiveTab);
        }
    }

    private void SelectTab(OpenNGSTabButton tabButton)
    {
    ...
    private void DeselectTab(OpenNGSTabButton tabButton)
    {
        VisualElement target = tabButton.Target;
    ...
    public void Activate(OpenNGSTabButton button)
    {
        if(m_ActiveTab != null)
        {
        ...
        OnSelect?.Invoke(m_ActiveTab.Target);
    }
}
