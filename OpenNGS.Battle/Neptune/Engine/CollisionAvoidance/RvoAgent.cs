...
namespace Neptune
{
    public class RvoAgent : OrcaAgent
    {
        ...
        public RvoAgent(Actor ele, UVector2 pos, int radius, int maxSpeed, float timeHorizon, float timeHorizonObst, int neighborDist, int neighborDistObst, int id, RvoContainType rvoContainType) : base()
        {
            this.Owner = ele;
            this.Position = pos;
        ...
        protected override void ComputeNewVelocity(float dt)
        {
            ...
            for (int i = 0; i < obstNeighborCount; i++)
            {
                ...
                for (int j = 0; j < orcaLineCount; j++)
                {
                    ...
                    if (MathUtil.Det((Vector2)(relativePosition1 * invTimeHorizonObst - orcaLine.Point), orcaLine.Direction) - invTimeHorizonObst * selfRadius >= NeptuneConst.RVO_EPSILON
                        && MathUtil.Det((Vector2)(relativePosition2 * invTimeHorizonObst - orcaLine.Point), orcaLine.Direction) - invTimeHorizonObst * selfRadius >= NeptuneConst.RVO_EPSILON)
                    {
#if BATTLE_LOG
                        if (CanOutputBattlelog())
                ...
                if (s < .0f && distSq1 <= radiusSq)
                {
                    ...
                    if (obstacle1.Convex)
                    {
                        line.Point = NeptuneConst.Vector2Zero;
                ...
                if ((t < .0f - NeptuneConst.RVO_Fault_Tolerance && tLeft < .0f) || (obstacle1 == obstacle2 && tLeft < .0f && tRight < .0f))
                {
                ...
                else if (t > 1.0f + NeptuneConst.RVO_Fault_Tolerance && tRight < .0f)
                {
            ...
            if (obstNeighborCount > 0 && NeptuneConst.RVO_ForceSpeed && this.RvoContainType == RvoContainType.AgentHero)
            {
                newVelocity = newVelocity.normalized * maxSpeed;
        ...
        private bool LinearProgram1(List<OrcaLine> lines, int lineNo, int radius, Vector2 optVelocity, Vector2 tempVelocity, bool directionOpt, ref UVector2 result)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    #region Next version
                    //if (NeptuneConst.RVO_ForceSpeed && this.RvoContainType == RvoContainType.AgentHero)
                    //{
...
#endif
                        if (Math.Abs(determinant) <= NeptuneConst.RVO_EPSILON)
                        {
